<?php

namespace App\Http\Controllers\Api\V1\Admin;

use App\Http\Controllers\Controller;
use App\Http\Requests\StoreGovernmentRequest;
use App\Http\Requests\UpdateGovernmentRequest;
use App\Http\Resources\Admin\GovernmentResource;
use App\Models\Government;
use Gate;
use Illuminate\Http\Request;
use Illuminate\Http\Response;

class GovernmentApiController extends Controller
{
    public function index()
    {
        abort_if(Gate::denies('government_access'), Response::HTTP_FORBIDDEN, '403 Forbidden');

        return new GovernmentResource(Government::with(['country'])->get());
    }

    public function store(StoreGovernmentRequest $request)
    {
        $government = Government::create($request->validated());

        return (new GovernmentResource($government))
            ->response()
            ->setStatusCode(Response::HTTP_CREATED);
    }

    public function show(Government $government)
    {
        abort_if(Gate::denies('government_show'), Response::HTTP_FORBIDDEN, '403 Forbidden');

        return new GovernmentResource($government->load(['country']));
    }

    public function update(UpdateGovernmentRequest $request, Government $government)
    {
        $government->update($request->validated());

        return (new GovernmentResource($government))
            ->response()
            ->setStatusCode(Response::HTTP_ACCEPTED);
    }

    public function destroy(Government $government)
    {
        abort_if(Gate::denies('government_delete'), Response::HTTP_FORBIDDEN, '403 Forbidden');

        $government->delete();

        return response(null, Response::HTTP_NO_CONTENT);
    }
}
