<form wire:submit.prevent="submit" class="pt-3">
    ...
    <div class="form-group {{ $errors->has('ticket.type') ? 'invalid' : '' }}">
        <label class="form-label required">{{ trans('cruds.ticket.fields.type') }}</label>
        <select class="form-control" wire:model="ticket.type">
            <option value="null" disabled>{{ trans('global.pleaseSelect') }}...</option>
        ...
        <div class="validation-message">
            {{ $errors->first('ticket.type') }}
        </div>
        <div class="help-block">
            {{ trans('cruds.ticket.fields.type_helper') }}
        </div>
    </div>
